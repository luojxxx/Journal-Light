...
class Gallery extends React.Component{
    ...
    handleDeleteMedia(){
            ...
            var file = this.props.photos[this.state.currentImage].lightboxImage.file;
            if (this.lightboxImages.length === 1){
                this.closeLightbox();
            } else {
                var goToImage = 0;
                if (this.state.currentImage+1 < this.lightboxImages.length){
                    goToImage = this.state.currentImage
                } else {
                    goToImage = this.state.currentImage-1
                }
                this.setState({currentImage: goToImage});
            }
            
            ChatAppActions.deleteMediaFromEntry(dataId, file.fileName);

            if (file.fileType === 'video') {
                ChatAppActions.deleteMediaFromEntry(dataId, file.realFileName);
            };
    }

    deleteButton() {
        return <button onClick={this.handleDeleteMedia}> Delete </button>
    }
	...
	if (this.props.disableLightbox){
		...
		<div id="Gallery" className="clearfix" ref={(c) => this._gallery = c}>
			...
			width={1600}
                                    customControls={ [this.deleteButton()] } 
